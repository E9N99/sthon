#all write Codes By Team SedUb @NUNJJ
#By Hussein @NUNUU
import asyncio
import random
import json
...
async def jepvois(vois):
  ...
  await vois.client.send_file(vois.chat_id,url,caption="᯽︙ BY : @X_X_X_X_R 🤲🏻☪️",parse_mode="html")
  await vois.delete()
@l313l.on(admin_cmd(outgoing=True, pattern="ثيم$"))
async def jepThe(theme):
  rl = random.randint(2,510)
  url = f"https://t.me/GSSSD/{rl}"
  await theme.client.send_file(theme.chat_id,url,caption="᯽︙ THEME BY : @X_X_X_X_R 🎊",parse_mode="html")
  await theme.delete()
...
async def Hussein(event):
    ...
    sedub = await reply_id(event)
    if sed in sedthon_links:
        url = sedthon_links[sed]
        await event.client.send_file(event.chat_id, url, parse_mode="html", reply_to=sedub)
        await event.delete()
